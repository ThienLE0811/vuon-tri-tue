...
export function RegisterForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    toast.success("Đăng ký thành công! Bạn hãy đăng nhập để bắt đầu học nhé 🌟");
    router.push("/login");
  }
        ...
        <CardContent className="space-y-4">
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="name" className="font-bold text-slate-700">Họ và tên hoặc Tên hiển thị</Label>
              <Input
                id="name"
                required
                placeholder="Ví dụ: Minh Khôi hoặc Bé Bo"
                disabled={loading}
